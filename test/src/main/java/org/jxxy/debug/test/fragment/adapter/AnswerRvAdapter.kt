...
import org.jxxy.debug.test.databinding.ItemAnswerBinding
...
class AnswerRvAdapter(val type:Int,val viewModel: QuestionViewModel? = null) : SingleTypeAdapter<CollectionOrMistakeQuestion>() {
    init {
    ...
    inner class AnswerRvViewHolder(view : ItemAnswerBinding) : SingleViewHolder<ItemAnswerBinding,CollectionOrMistakeQuestion>(view){
        override fun setHolder(entity: CollectionOrMistakeQuestion) {
            if (entity.questionText.length >= 20) {
                view.questionContentsTv.text = entity.questionText.substring(0, 20) + "..."
            }else{
                view.questionContentsTv.text = entity.questionText
            }
            if(entity.questionAnswerCount == 0){
                view.isDoneIcon.hide()
            }else{
                view.isDoneIcon.show()
            }
...
//            }
    ...
    override fun createViewHolder(
        ...
        return AnswerRvViewHolder(ItemAnswerBinding.inflate(inflater,parent,false))
    }
}